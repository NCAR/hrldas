...
#This program assimilated soil moisture, leaf area index(LAI), and solar-induced chlorophyll fluorescence (SIF) observations to the Noah-MP-Crop model
...
for ii in range(0,istep):
    ...
    if DA_LAI_2==True:
        ...
        if obs_lai[0,ii+1]!=-999:
            ...
            #The error covariance of leaf biomass estimates
            epsilon1=en_sim_lfmass-(en_sim_lfmass).mean()
            diff1=np.square(epsilon1).sum()
            #The error covariance of leaf biomass observations
            epsilon2=en_obs_lai_lfmass-obs_lai_lfmass
            diff2=np.square(epsilon2).sum()
            #The error covariance matrix between leaf biomass estimates and observations
