...
err_soilm_o=0.1                                        # observation error of soil moisture,               Adjustable!
...
obs_lai=np.asmatrix(observations[3])                       # leaf area index observation
obs_soilm_1=np.asmatrix(observations[4])                   # surface soil moisture observation
obs_sif=np.asmatrix(observations[6])                       # solar-induced Fluorescence observation
...
for ii in range(0,istep):
    ...
    # Read initial leaf biomass and soil moisture
    for i in range(0,en_numb):
    ...
    if DA_LAI_2==True:
        ...
        if obs_lai[0,ii+1]!=-999:
            ...
            en_obs_lai=obs_lai[0,ii+1]+obs_lai[0,ii+1]*err_lai_o*np.random.randn(en_numb)
            en_obs_lai=np.asmatrix(en_obs_lai).T
            en_obs_lai_lfmass=en_obs_lai/BIO2LAI            
    ...
    if DA_SIF==True:
        ...
        if obs_sif[0,ii]!=-999:
            ...
            MPTABLE_data_line=" VCMX25=  50.0,   60.0,   60.0,   60.0,   55.0,   40.0,   40.0,   40.0,   40.0,   40.0,   50.0,   "+ str(('%.1f'%vcmax25_num_1))+ ",   0.00,   60.0,   0.00,   0.00,   0.00,   50.0,   50.0,   50.0,"+"\n"
